 ...
 * @param impulseId
 */
export const storeApiKeyAndImpulseId = (apiKey, impulseId) => {
...
export const getKeyword = () => new URLSearchParams(window.location.search).get('keyword') ||
    localStorage.getItem(LS_KEYWORD) ||
    '';
export const storeKeyword = (keyword) => {
    console.log('storeKeyword', keyword, window.location.search);
    localStorage.setItem(LS_KEYWORD, keyword);
};
export const getLabel = () => new URLSearchParams(window.location.search).get('label') ||
...
export const getFrequency = () => Number(new URLSearchParams(window.location.search).get('frequency')) ||
    Number(localStorage.getItem(LS_FREQUENCY)) ||
    NaN;
export const storeFrequency = (frequency) => {
    console.log('storeFrequency', frequency, window.location.search);
    localStorage.setItem(LS_FREQUENCY, frequency.toString());
};
export const getSampleLength = () => Number(new URLSearchParams(window.location.search).get('sampleLength')) ||
    Number(localStorage.getItem(LS_SAMPLE_LENGTH)) ||
    NaN;
export const storeSampleLength = (sampleLength) => {
...
export const getDeviceId = () => localStorage.getItem(LS_DEVICE_ID_KEY) || `${devicePrefix}_${getRandomString()}`;
...
export const storeIngestionApi = (ingestionApi) => {
...
export const storeRemoteManagementEndpoint = (remoteManagementEndpoint) => {
...
export const storeStudioEndpoint = (studioEndpoint) => {
