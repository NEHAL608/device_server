...
export class ClassificationClientViews {
    constructor() {
        ...
        this._elements = {
            ...
            perfCalHint: document.querySelector('#performance-calibration-hint'),
            ...
            shareHint: document.querySelector('#inferencing-public-project-hint')
        };
        this._sensors = [];
    ...
    grantPermission() {
        ...
        sensor.checkPermissions(true).then(result => {
            if (result) {
                ...
                if (prop.sensor === 'camera') {
                    return this.sampleImagesContinuous(sensor, prop);
                }
    ...
    async sampleAudioContinuous(sensor, prop) {
        if (!sensor || !this._classifier)
            return;
        ...
        const classify = async (data) => {
            try {
                ...
                if (prop.continuousMode) {
                    ...
                    res = this._classifier.classifyContinuous(data.slice(data.length - prop.continuousMode.sliceSize), false, true);
                }
                else {
    ...
    async sampleImagesContinuous(sensor, prop) {
        if (!this._classifier)
            return;
    ...
    async renderInferenceResults(res, prop, opts) {
        var _a;
        ...
        if (!this._isObjectDetection && res.results.length > 0) {
            ...
            if (res.results.length === 0 && res.visual_ad_grid_cells.length === 0 &&
                ...
                Notify.notify('', 'No objects found', 'top', 'center', 'fas fa-exclamation-triangle', 'success');
            }
...
//# sourceMappingURL=classification-views.js.map